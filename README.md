...
ROC_AUC was selected as the evaluation metric (see this [notebook](https://github.com/yinanli617/ctr-prediction/blob/master/compute_roc_auc.ipynb) for the reasons). After 15 epochs of training on a local GPU, the wide and deep model results in ROC_AUC of 0.7497 - a significant improvement (~5.4%) from gradient boosting tree model trained in Spark which has an ROC_AUC score of 0.7112.
