...
maxBins = 100
wide_cols = list(map(lambda c: c[0], filter(
    lambda c: c[1] <= maxBins, strColsCount)))
wide_col_counts = list(map(lambda c: c[1], filter(
    lambda c: c[1] <= maxBins, strColsCount)))
wide_cols += list(map(lambda c: c[0],
...
strIndexers_wide = list(map(lambda c: StringIndexer(
    inputCol=c, outputCol=c+'_idx'), wide_cols))

embed_features = map(lambda c: c[0] + 'SEP' +
                     str(c[1]) + 'SEP' + str(c[2]), embed_cols)
strIndexers_embed = list(map(lambda c: StringIndexer(inputCol=c[0],
                                                     outputCol=c[0] + 'SEP' +
                                                     str(c[1]) +
                                                     'SEP' + str(c[2])
                                                     ), embed_cols))
oneHotEncoders = list(map(lambda c: OneHotEncoder(
    inputCol=c+'_idx', outputCol=c+'_onehot'), wide_cols))

# Assemble the put as the input to the VectorAssembler
#   with the output being our features
vectorAssembler = VectorAssembler(inputCols=list(
    map(lambda c: c+'_onehot', wide_cols)), outputCol='wide_features_v')
